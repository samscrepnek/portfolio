const Background = () => {
  class Particle {
    ...
    move() {
      ...
      // console.log(`${"translateX(" + left + "px) translateY(" + top + "px) scale(" + this.scale + ") rotate(" + this.rotation + (this.position + this.dimensions.height) + "deg)"}`);
      this.svg.style.color = "blue";
      ...
      // this.item.style.transform = "scale(" + this.scale + ")";
      // this.item.style.transform = "rotate(" + this.rotation + (this.position + this.dimensions.height) + "deg)";
    ...
    render() {
      ...
      document.querySelector("body").append(this.item);
      return {
        width: this.item.innerWidth,
        height: this.item.innerHeight,
      };
    }
  }
